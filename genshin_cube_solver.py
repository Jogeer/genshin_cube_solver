...
class Field():
    ...
    def _check(self) -> bool:
        fsum = sum([cube.get_weight() for cube in self._field[:self._matrix_one_size]])

        for k in range(self._matrix_one_size):
            temp = sum([cube.get_weight() for cube in self._field[k*self._matrix_one_size:k*self._matrix_one_size+self._matrix_one_size]])
            if temp != fsum:
                return False

        for k in range(self._matrix_one_size):
            temp = sum([cube.get_weight() for cube in self._field[k::self._matrix_one_size]])
            ver = sum([self._field[k].get_weight(), self._field[k+self._matrix_one_size].get_weight(), self._field[k+self._matrix_one_size*2].get_weight()])
            if temp != fsum:
                return False

        dig1 = sum([cube.get_weight() for cube in self._field[0::self._matrix_one_size+1]])
        if dig1 != fsum:
            return False

        dig1 = sum([cube.get_weight() for cube in self._field[2:-1:self._matrix_one_size-1]])
        if dig1 != fsum:
            return False
...
def main() -> None: 
    ...
    Field(userStateMatrix, userDirMatrix).slaver()
